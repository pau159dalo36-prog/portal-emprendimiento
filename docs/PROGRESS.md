...
- ✅ **PASO 9**: panel `/panel/videos` con secciones por estado y acciones
  condicionadas (publicar solo si aprobado; retirar/archivar/desarchivar/eliminar).
...
- ✅ **PASO 11**: vídeos en proyecto, perfil y organización; `VIDEO_WITH_DETAILS`
  incluye `organization`; filtros explícitos `ready` + `approved`.
...
- Sin migraciones nuevas: el propietario no puede "reenviar a moderación" (el
  trigger exige admin para cambiar `moderation_status`). Los rechazados solo se
  pueden editar/eliminar.
- La publicación exige `moderation_status = 'approved'` en las server actions
  (además del `canPublishVideo` del panel), cerrando la invariante también fuera
  de la UI.
