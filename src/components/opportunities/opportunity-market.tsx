...
import { Briefcase, Loader2, RefreshCw, Search, X } from "lucide-react";
...
import type { SearchOpportunity, SearchPageResult } from "@/search/types";

type OpportunityMarketProps = {
  initialParams: MarketParams;
  initial: SearchPageResult<SearchOpportunity>;
};

// Mercado de oportunidades: una entidad con los filtros de la pestaña de /explorar
// pero sin pestañas. La URL es la fuente de verdad (query validada con Zod en el
// server); la página 1 llega como prop y "Cargar más" es client-side (cursor).
export function OpportunityMarket({ initialParams, initial }: OpportunityMarketProps) {
  const t = useTranslations("opportunity");
  const commonT = useTranslations("common");
  const typesT = useTranslations("opportunityTypes");
  const workModesT = useTranslations("workModes");
  const experienceT = useTranslations("experienceLevels");
  const industriesT = useTranslations("industries");
  const datesT = useTranslations("opportunityDates");
  const router = useRouter();

  const { q, sort, opportunityType, workMode, experience, industry, date, firstJob } = initialParams;
  const [input, setInput] = useState(q);
  const [items, setItems] = useState<SearchOpportunity[]>(
  ...
  const navigate = useCallback(
    (next: Partial<MarketParams>) => {
      ...
      router.push(query ? { pathname: "/oportunidades", query } : "/oportunidades");
    },
  ...
  const loader = useCallback(
    ...
    [q, sort, opportunityType, industry, workMode, experience, firstJob, date],
  );
  ...
  const retry = useCallback(() => {
    ...
    loader(null).then(
      ...
      () => {
        ...
        setLoading(false);
        setInitialError("opportunity.loadError");
      },
    );
  ...
  if (opportunityType) activeChips.push({ key: "opportunityType", label: typesT(opportunityType as never), clear: { opportunityType: "" } });
  if (workMode) activeChips.push({ key: "workMode", label: workModesT(workMode as never), clear: { workMode: "" } });
  if (experience) activeChips.push({ key: "experience", label: experienceT(experience as never), clear: { experience: "" } });
  if (industry) activeChips.push({ key: "industry", label: industriesT(industry), clear: { industry: "" } });
  if (date) activeChips.push({ key: "date", label: datesT(date as never), clear: { date: "" } });

  function clearAllFilters() {
    navigate({ opportunityType: "", workMode: "", experience: "", industry: "", date: "", firstJob: "false" });
  }
      ...
      {items.length === 0 ? (
        <section className="grid gap-6 rounded-2xl border border-dashed border-border/70 bg-card p-8 text-center">
          <div className="mx-auto flex size-14 items-center justify-center rounded-2xl bg-primary/10 text-primary">
            {q ? <Search className="size-7" aria-hidden="true" /> : <Briefcase className="size-7" aria-hidden="true" />}
          </div>
