...
export async function moderateServiceAction(
  ...
  const t = await getTranslations("actions.serviceModeration");
  ...
  const rejectionArgs: { p_service_id: string; p_reason?: string } = {
  ...
  const { error } =
    intent === "approve"
      ? await supabase.rpc("admin_approve_service", { p_service_id: serviceId })
      : intent === "reject"
        ? await supabase.rpc("admin_reject_service", rejectionArgs)
        : await supabase.rpc("admin_flag_service", rejectionArgs);
